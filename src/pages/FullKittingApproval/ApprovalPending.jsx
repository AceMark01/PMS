  ...
  const renderRow = (item, idx) => {
    ...
    return (
      <tr key={item.id || idx} className="hover:bg-indigo-50/30 transition-colors border-b border-gray-100">
        {/* Action: Open Approval Review Modal */}
        <td className="px-4 py-3 text-center whitespace-nowrap text-xs">
          <button
            onClick={() => onOpenApprovalForm(item.id)}
            className="px-2.5 py-1 text-[10px] font-bold bg-indigo-50 text-indigo-600 border border-indigo-100 rounded hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center gap-1 active:scale-95 mx-auto"
            title="Review Costing Card"
          >
            <ShieldAlert size={12} />
            <span>Review</span>
          </button>
    ...
    return (
      <div key={item.id || idx} className="bg-white rounded-xl border border-indigo-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-indigo-100">
        <div className="flex justify-between items-center pb-2 border-b border-slate-50">
          ...
          <button
            onClick={() => onOpenApprovalForm(item.id)}
            className="px-2.5 py-1 text-[10px] font-bold bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-md hover:bg-indigo-600 hover:text-white transition flex items-center gap-1 active:scale-95"
        ...
        <div className="grid grid-cols-2 gap-x-2 gap-y-1.5 text-[11px] bg-slate-50 rounded-lg p-2.5 border border-slate-100/50">
          ...
          <div className="col-span-2 border-t border-slate-100 pt-1">
            ...
            <span className="text-gray-700 font-semibold">{item.rawNames}</span>
          </div>
          <div>
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Production Cost</span>
