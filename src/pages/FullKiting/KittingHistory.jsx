...
export default function KittingHistory({ data, visibleColumns = [] }) {
  ...
  const allHeaders = [
    "S NO", "Timestamp", "Product code", "Product Name", "BAse Cat", "Order Quantity", "GoDown", "Planned 1", "Actual 1"
  ];
  ...
  const renderRow = (item, idx) => {
    return (
      <tr key={item.id || idx} className="hover:bg-indigo-50/30 transition-colors border-b border-gray-100">
        ...
        <td className="px-4 py-3 text-center text-xs text-slate-700 whitespace-nowrap">{item.actual1 || '-'}</td>
      </tr>
    );
  };
      ...
      <div key={item.id || idx} className="bg-white rounded-xl border border-indigo-50 shadow-sm p-4 space-y-3 transition-all hover:shadow-md hover:border-indigo-100">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 rounded-lg p-2 border border-slate-100/50">
          ...
          <div className="col-span-2">
            <span className="text-gray-400 block uppercase text-[8px] tracking-tight">Actual 1</span>
            <span className="text-gray-700 font-medium">{item.actual1 || '-'}</span>
          </div>
        </div>
      </div>
    );
